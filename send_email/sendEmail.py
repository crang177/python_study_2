...
def sendEmail_2():
    ...
    email_body="my name is rg"#文件主体的内容
    #附加列表
    email_attachment=['a.txt',]


    #连接服务器
    #创建yafmail.SMTP对象
    yag=yagmail.SMTP(user,pass_code,host)

    #发送邮件yagmail的send方法
    yag.send(email_to,title,email_body,email_attachment)
    #关闭连接
    yag.close()

schedule.every().day.at("20:20").do(sendEmail_2())
while True:
    # 这是  schedule  库的一个方法，用于检查并执行所有待执行的任务。如果有任务的执行时间到了，它会立即执行这些任务。
    schedule.run_pending()
